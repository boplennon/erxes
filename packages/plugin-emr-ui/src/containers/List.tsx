...
type FinalProps = {
  listQuery: EmrQueryResponse;
  listEmrTypeQuery: TypeQueryResponse;
} & Props &
  RemoveMutationResponse &
  EditMutationResponse;
...
const ListContainer = (props: FinalProps) => {
  ...
  const types = listEmrTypeQuery.emrTypes || [];
  ...
  const remove = emr => {
    confirm('You are about to delete the item. Are you sure? ')
      .then(() => {
        removeMutation({ variables: { _id: emr._id } })
          .then(() => {
  ...
  const updatedProps = {
    ...props,
    emrs: listQuery.emrs || [],
    types: listEmrTypeQuery.emrTypes || [],
    typeId,
...
export default withProps<Props>(
  compose(
    graphql(gql(queries.listEmrTypes), {
      name: 'listEmrTypeQuery',
      options: () => ({
        fetchPolicy: 'network-only'
      })
    }),

    graphql<Props, EmrQueryResponse, { typeId: string }>(gql(queries.list), {
      name: 'listQuery',
      options: ({ typeId }) => ({
