...
const stringeeQueries = {
  async stringeeGetAccounts(_root, { kind }: IKind, { models }: IContext) {
    return models.Accounts.find({ kind });
  },

  async stringeeGetIntegrations(_root, { kind }: IKind, { models }: IContext) {
    return models.Integrations.find({ kind });
  },

  async stringeeGetIntegrationDetail(
    _root,
  ...
  async stringeeConversationMessages(
    _root,
  ...
  async stringeeConversationMessagesCount(
    _root,
  ...
  async stringeeHasTaggedMessages(
    _root,
...
export default stringeeQueries;
